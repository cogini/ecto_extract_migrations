  ...
  def parse(sql) do
    case parsec_create_table(sql) do
      {:ok, value, _, _, _, _} ->
        ...
        {:ok, Map.put(attrs, :columns, Enum.map(Enum.reverse(columns), &fix_column/1))}
      error -> error
    end
  end

# :primary_key - when true, marks this field as the primary key. If multiple
#   fields are marked, a composite primary key will be created.
...
# :size - the size of the type (for example, the number of characters). The
#   default is no size, except for :string, which defaults to 255.
